...
interface IERC20 {
    ...
    emite un eveto{Transfer].*/
    function transfer(address to,uint256 amount) external returns (bool);

    /*Devuelve el numero restante de tokens que se le permitira al 'spender'
    gastar en nombre del 'owner' a traves de {transferForm}.Este valor es cero por defecto.Este valor cambia cuando
    {transferFrom}.Este valor es cero por defecto.Este valor cambia cuando{approve} o {transferFrom} son llamadas.*/ 
    function allowance(address owner,address spender) external view returns(uint256);

    /*Establece una 'amount' como la asignacion de 'spender sobre los tokens de la persona que llama.devuelve un valor 
    booleano que indica s i la operacion tuvo exito.
